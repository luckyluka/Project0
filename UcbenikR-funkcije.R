...
#izračuna frekvenčno porazdelitev s % in kumulativami
...
#izračuna korelacijsko matriko skupaj s številom enot in stopnjo značilnosti
...
#"lepo" izpiše rezultat zgornje funkcije
...
#vstavi manjkajoče vrednosti kjer so bile izločene iz podatkov s funkcijo "na.omit" ali podobno ali na mesta, ki so podana v argumentu
...
#izračuna vsoto kvadratnih odklonov od popvrečja
ss<-function(x){
    m<-mean(x)
    sum((m-x)^2)
}

#izračuna skupno vsoto kvadratnih odklonov od popvrečja za vse spremenljivke
ssAllVar<-function(x){
    sum(apply(x,2,ss))
}

# za neko razbitje in spremenljivke izračuna vrednost Wardove kriterijske funkcije
...
#funkcija za računanje lastnih vrednosti in indeksov pogojnosti
	X<-fit["x"]
	if(is.null(X)){
...
# funkcija za risanje povprečji skupin pri večih spremenljivkah
plotMeans<-function(x, by, plotCI=TRUE,alpha=0.05,ylab="povprečja",plotLegend=TRUE,inset = 0.01, xleg="topleft", legPar=list(),gap=0, ...){  
    tmpM<-aggregate(x =x,by = by, FUN=mean, na.rm=TRUE)
    matplot(t(tmpM[,-1]),type="o",xaxt="n",ylab=ylab,...)
    axis(side=1,at=1:dim(tmpM[,-1])[2],labels=colnames(tmpM)[-1],las=2)
    g<-dim(tmpM)[1]
    res<-list(means=tmpM)
    if(plotLegend)  {
